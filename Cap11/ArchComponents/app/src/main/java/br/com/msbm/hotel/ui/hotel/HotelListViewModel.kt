...
class HotelListViewModel(private val hotelRepository: HotelRepository) : ViewModel() {
    ...
    fun deleteSelected() {
        hotelRepository.remove(*selectedItems.toTypedArray())
        deletedItems.clear()
    ...
    private fun toggleHotelSelected(hotel: Hotel) {
        val existing = selectedItems.find { hotel ->
            hotel.id == hotel.id
        }
